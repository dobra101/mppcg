...
:- avl_map(det, ?) is det.  % This is the expected mode, a determinate producer.
:- avl_map(semidet, ?) is semidet.
:- avl_map(multi, ?) is multi.
:- avl_map(failing, ?) is semidet. % (!) Pred will not be called on empty input.
:- avl_map(nondet, ?) is nondet.
