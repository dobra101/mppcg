...
class Generator {
    ...
    // maps an identifier name to nodes which use this identifier
    // -> used for keyword handling
    ...
    fun generate(start: Start, outputPath: String): File {
        // convert the AST to intermediate code
        val program: Program = start.convert()

        // preprocessing
        TypeInference.inferTypes(program)
        program.handleKeywords()

        // recursively render the intermediate code
    ...
    /**
     * Traverses all child nodes via reflection and collects [NamedNode]s in the identifierReferenceMap.
     */
