...
proc ctext::highlightAfterIdle {win lineStart lineEnd} {
	...
	if {"" eq $configAr(highlightAfterId)} {
                regexp {([0-9]+)\.[0-9]+} $lineStart match startline
                regexp {([0-9]+)\.[0-9]+} $lineEnd match endline
		set currentline $startline
		while {$currentline <= $endline} {
	                if {[string length [$win get $currentline.0 $currentline.end]] > 500} {
	     		     set configAr(highlightAfterId) [after idle [list ctext::highlight $win $currentline.0 $currentline.500 [set afterTriggered 1]]]
	                } else {
	     		     set configAr(highlightAfterId) [after idle [list ctext::highlight $win $currentline.0 $currentline.end [set afterTriggered 1]]]
	                }
			incr currentline
		}
	}
...
proc ctext::instanceCmd {self cmd args} {
	...
	delete {
		...
		foreach tag [$self._t tag names] {
			...
			regexp {([0-9]+)\.[0-9]} $removeStart match start
			regexp {([0-9]+)\.[0-9]} $removeEnd match end
			if {$start +1  == $end} {
				set removeStart $end.0
		...
		regexp {([0-9]+)\.[0-9]+} $lineStart match linestart
		regexp {([0-9]+)\.[0-9]+} $lineEnd match lineend
 		if {$linestart +1 == $lineend} {set lineStart $lineend.0}
	...
	insert {
	    ...
	    foreach tag [$self._t tag names] {
		if {[string equal $tag "_cComment"] != 1} {
			regexp {([0-9]+)\.[0-9]} $prevSpace match start
			regexp {([0-9]+)\.[0-9]} $nextSpace match end
			if {$start +1  == $end} {
				set prevSpace $end.0
		...
		ctext::commentsAfterIdle $self
  	    ...
  	    regexp {([0-9]+)\.[0-9]} $lineStart match start
	    regexp {([0-9]+)\.[0-9]} $lineEnd match end
	    if {$start +1 == $end} {
		    ctext::highlightAfterIdle $self $end.0 $lineEnd
	    } else {
		    ctext::highlightAfterIdle $self $lineStart $lineEnd
	    }
	    switch -- $data {
...
proc ctext::highlight {win start end {afterTriggered 0}} {
    ...
    regexp {([0-9]+)\.[0-9]+} $start m firstLine
    if [regexp {([0-9]+)\.[0-9]+} $ei m lastLine] {
        regexp {([0-9]+)\.[0-9]+} [$twin index end] match lLineInText
        if {$lastLine != $lLineInText && $lastLine != $firstLine} {
            return
        }
    }
