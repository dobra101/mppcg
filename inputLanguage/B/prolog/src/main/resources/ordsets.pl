...
%@  a name at all is that you may not have realized that @code{sort/2} could be
%@  used this way.
:- list_to_ord_set/2 is det.
...
:- ord_disjoint_union/3 is semidet. % perhaps det is the more common mode. Also see sets:disjoint_union/3.
...
:- ordset_order/3 is semidet.   % (fails if incomparable)
